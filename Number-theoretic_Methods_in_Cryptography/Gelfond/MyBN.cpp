...
BN BN::root(int n) {
    ...
    do{
        x0 = x1;
        x1 = (x0 * (n - 1) + a/x0.pow_int(n-1)) * 1/n;
    }
    while (x0 > x1);
    return x0;
}
...
BN BN::gcd(BN num){
    ...
    BN c;
    int i = 0, j = 0, k;
    while(a%2 == zero && a != zero){
        a = a / 2;
        i++;
    }
    while(b%2 == zero && b != zero){
        b = b / 2;
        j++;
    }
    if(i > j)
...
BN BN::random_bound(BN lower, BN higher){
...
bool BN::Miller_Rabin_test(int t){
    BN n = *this;
    BN zero, two, one;
    one = 1;
    two = 2;
